...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(
    ["1. Probabilistic technology", "2. Rational Behavior", "3. Results", "4. Graphs", "5. Description", "Readme"]
)
...
with tab6:
    st.markdown("## 📖 Readme: Quick Start & User Guide")
    st.markdown(
        r"""
        This Streamlit application simulates the dynamic Bayesian game of kidnapping and counter-kidnapping policy
        described in **Bernal, H. (2026): "Identifying Rational Types in Unknown Environments"**.

        To generate and analyze simulation results, follow this simple step-by-step guide. It explains exactly which tab and section contains each manual input parameter, complete with a visual representation of the controls.
        """
    )
    
    st.markdown("### 🛠️ Mandatory Configuration & Execution Steps")
    st.markdown(
        r"""
        1. **Step 1: Set Environmental Covariates (Tab 1 ➔ Block B)**
           * **Where to find it:** Pestaña/Tab **`1. Probabilistic technology`** ➔ Section **`## 🅱️ Competing Risks & Effective Intensities`** ➔ *Modifiable Covariates* subsection.
           * **What to adjust:** Select the perpetrator type (*Perpetrator*), victim profile (*Victim profile*), family wealth (*Family Wealth*), geographic zone (*Geographic zone*), and state capacity type (*State type*).
           * **Visual Guide:**
           ```text
           +--------------------------------------------------------+
           |  Tab: 1. Probabilistic technology -> Block B           |
           +--------------------------------------------------------+
           |  Perpetrator:      [ FARC        v ]                   |
           |  Victim profile:   [ Private     v ]                   |
           |  Family wealth:    [ Standard    v ]                   |
           |  Geographic zone:  [ Metropolis  v ]                   |
           |  State type:       [ Strict      v ]                   |
           +--------------------------------------------------------+
           ```
        
        2. **Step 2: Configure Frictional Action Settings & Latent Intentions (Tab 1 ➔ Block A)**
           * **Where to find it:** Pestaña/Tab **`1. Probabilistic technology`** ➔ Section **`## 🅰️ The Mano de Dios-Guadalupe (MDG) Frictional Process`** (at the very top of Tab 1).
           * **What to adjust:** 
             * Use the sliders to configure the noise temperatures for each player ($T_0^S, T_0^F, T_0^K$) and the noise decay parameters ($\eta_{\text{cal}}$, $\bar{c}$, $H_{\text{ratio}}$).
             * Select the baseline latent intentions (*State Latent Intention*, *Captor Latent Intention*, *Family Latent Intention*) in the dropdown menus to test single-period ($\tau=0$) simulations.
           * **Visual Guide:**
           ```text
           +--------------------------------------------------------+
           |  Tab: 1. Probabilistic technology -> Block A           |
           +--------------------------------------------------------+
           |  T0_S:   [ ---o-------------- ]  0.75                  |
           |  T0_F:   [ ---o-------------- ]  0.75                  |
           |  T0_K:   [ ---o-------------- ]  0.75                  |
           |  c_bar:  [ --o--------------- ]  0.05                  |
           |                                                        |
           |  State Latent Intention:    [ Negotiate  v ]           |
           |  Captor Latent Intention:   [ Continue   v ]           |
           |  Family Latent Intention:   [ Cooperate  v ]           |
           +--------------------------------------------------------+
           ```
        
        3. **Step 3: Define Initial Bayesian Prior (Prior $\mu_0$) (Tab 1 ➔ Section 7)**
           * **Where to find it:** Pestaña/Tab **`1. Probabilistic technology`** ➔ Section **`## 7️⃣ Initial Bayesian Prior (μ₀) over Captor Type`** (at the bottom of Tab 1).
           * **What to adjust:** Adjust the sliders for the initial belief over the perpetrator types: Common Crime ($\mu_0(\text{DC})$), Paramilitaries ($\mu_0(\text{PAR})$), and ELN ($\mu_0(\text{ELN})$). The FARC belief updates automatically to sum to $100\%$.
           * **Visual Guide:**
           ```text
           +--------------------------------------------------------+
           |  Tab: 1. Probabilistic technology -> Section 7         |
           +--------------------------------------------------------+
           |  u0(DC):   [ ---------o-------- ]  0.55                |
           |  u0(PAR):  [ -o---------------- ]  0.05                |
           |  u0(ELN):  [ ------o----------- ]  0.35                |
        ...
        4. **Step 4: Define Horizon & Run Dynamic Optimization (Tab 3 ➔ Top Controls)**
           * **Where to find it:** Pestaña/Tab **`3. Results`** (at the top of the tab).
           * **What to adjust:** Set the maximum simulation horizon $T_{\text{max}}$ (default is 20) and toggle the *Counterfactual extension* option.
           * **Execute:** Click the mandatory execution button `▶️ Run State Optimization (τ=1…T_max, dynamic — grid search + IC/IR masks + trained value net, T=10)` to run the backward-induction solver and populate the dynamic simulation log.
           * **Visual Guide:**
           ```text
           +--------------------------------------------------------+
           |  Tab: 3. Results (Top Section)                         |
           +--------------------------------------------------------+
           |  T_max:                 [ 20  +/- ]                    |
           |  Counterfactual ext.:   [X] Enable                     |
           |                                                        |
    ...
    st.markdown("### 📊 Output Analysis & Dynamic Visualization (Post-Execution)")
    st.markdown(
        r"""
        Once the optimization is run (Step 4), you can explore the following tabs to analyze the results:
        *   **Tab `3. Results`:** Displays the full dynamic history log. Expand the *Details at period τ* selectbox to inspect active learning entropy reductions, Bellman constraints, and grid objectives.
        *   **Tab `4. Graphs`:** Automatically plots the trajectories of beliefs ($\mu_\tau$), entropy ($H(\mu_\tau)$), expected entropy reduction ($\Delta H_\tau$), and policy controls ($\alpha_\tau^*$, $\gamma_\tau^*$).
        *   **Tab `5. Description`:** Look up the definition and mathematical formulas of any row variable in Table 5.2.
        """
    )
    st.info(
        "💡 **Resetting the Application**: To start a completely clean run, click the **'🔄 Reset'** button at the top right of **Tab 3**. "
        "This clears the active trajectory and resets all session state variables to prevent carry-over draws."
    )
