* ==============================================================================
* APPENDIX A REPLICATION: MICROSTRUCTURE AND COMPETING HAZARDS
...
display as text _n(2) "=== APPENDIX A REPLICATION ==="
...
* Save local values before modifying dataset
local val_farc = 1 - surv1[1]
local val_eln = 1 - surv2[1]
local val_para = 1 - surv3[1]
local val_del = 1 - surv4[1]

* 1. Export Survival_Rescate sheet (using whole dataset)
preserve
rename surv1 FARC
rename surv2 ELN
rename surv3 Paramilitares
rename surv4 Delincuencia
rename _t Dias
keep FARC ELN Paramilitares Delincuencia Dias
export excel using "`outdir'/Datos_Graficas_Cox.xlsx", sheet("Survival_Rescate") sheetreplace firstrow(variables)
restore

* 2. Write the .txt file (keep only terminal observation)
preserve
keep if _t == `tday'
...
* 3. Export Cumul_Rescue sheet (create new in-memory dataset)
clear
set obs 4
gen Grupo_Nombre = ""
gen Pr_Rescue_day_98 = .
replace Grupo_Nombre = "FARC" in 1
replace Pr_Rescue_day_98 = `val_farc' in 1
replace Grupo_Nombre = "ELN" in 2
replace Pr_Rescue_day_98 = `val_eln' in 2
replace Grupo_Nombre = "Paramilitares" in 3
replace Pr_Rescue_day_98 = `val_para' in 3
replace Grupo_Nombre = "Delincuencia" in 4
replace Pr_Rescue_day_98 = `val_del' in 4
rename Pr_Rescue_day_98 Pr_Rescue_day_`tday'

export excel using "`outdir'/Datos_Graficas_Cox.xlsx", sheet("Cumul_Rescue") sheetreplace firstrow(variables)
